...
async def run_pipeline(workflow_id: str, input_data: dict):
    try:
        ...
        # 4. Strategy
        platform = input_data.get("type", "blog")
        log_message(workflow_id, "[Strategy] Adding strategy...")
        strategy = await strategy_agent(compliance, platform)
        insert_workflow_step(workflow_id, "strategy", compliance, strategy)
        ...
        localized = await localization_agent(final, language)
        insert_workflow_step(workflow_id, "localization", final, localized)
